...
import { getContext, getWebSocket, sendWsBytesExport } from './app.js';
import {
    detectContentType,
...
interface ClipboardState {
    lastSharedTime: number;
    lastSharedType: string;
    lastSharedSize: number;
    isReceiving: boolean;
...
export function initClipboardUI(): void {
    // Wire fallback textarea paste handler
    const fallbackArea = document.getElementById('clipboard-fallback-area') as HTMLTextAreaElement;
    if (fallbackArea) {
        fallbackArea.addEventListener('paste', async (e: ClipboardEvent) => {
            e.preventDefault();
 ...
 */
export async function shareClipboard(): Promise<void> {
    ...
    try {
        ...
        if (statusEl) statusEl.textContent = 'Clipboard is empty.';
    } catch (_err) {
        // Clipboard API denied -- show fallback textarea
        showClipboardFallback();
        if (statusEl) statusEl.textContent = 'Clipboard access denied. Paste content below.';
    }
}
 ...
 */
async function sendClipboardText(text: string): Promise<void> {
    const ctx = getContext();
    const statusEl = document.getElementById('clipboard-status');
 ...
 */
async function sendClipboardImage(imageData: Uint8Array): Promise<void> {
    const ctx = getContext();
    const statusEl = document.getElementById('clipboard-status');
 ...
 */
export function checkClipboardManifest(manifestBytes: Uint8Array): { isClipboard: boolean; contentType: string } | null {
    try {
        const result = parseClipboardContent(manifestBytes) as any;
...
function showClipboardFallback(): void {
    ...
    if (fallback) {
        ...
        const area = document.getElementById('clipboard-fallback-area') as HTMLTextAreaElement;
        if (area) area.focus();
    }
}

export function hideClipboardFallback(): void {
    const fallback = document.getElementById('clipboard-fallback');
    if (fallback) {
...
function showReceivedContent(data: Uint8Array, contentType: string): void {
