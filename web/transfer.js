...
import { blake3Hash, computeFileManifest, parseFileManifest, TransferSession, sanitizeDisplayText, } from './wasm.js';
// ============================================================================
// Constants
// ============================================================================
...
// Initialization
// ============================================================================
export function initTransferUI() {
    wireDropZone();
    wireFileInputs();
    wireSendButton();
    loadTransferHistory();
...
function showSizeWarning(type, message) {
 ...
 */
export async function handleFileSend() {
    const ctx = getContext();
    if (!ctx.transferSession || !ctx.ws)
        return;
    ...
    for (const file of state.files) {
        ...
        while (offset < file.size || (file.size === 0 && offset === 0)) {
            ...
            const chunkMsg = ctx.transferSession.prepareChunk(globalChunkIndex, total, plaintext);
...
// Receive Flow
// ============================================================================
export function handleFileOffer(offer) {
    const ctx = getContext();
...
export function handleChunkReceived(chunk) {
    const ctx = getContext();
...
export function handleTransferComplete(complete) {
    state.isReceiving = false;
    // Assemble file from chunks
    if (state.receiveManifest) {
        assembleAndDownload();
    }
    showTransferSummary('received', state.receiveManifest?.files?.length || 0, state.totalSize, Date.now() - state.startTime);
}
function assembleAndDownload() {
    if (!state.receiveManifest || !state.receiveManifest.files)
        return;
...
// Progress Display
// ============================================================================
function updateProgress(chunksComplete, totalChunks, bytesTransferred) {
    const now = Date.now();
    // Throttle UI updates to ~4 per second
    if (now - state.lastUpdateTime < 250 && chunksComplete < totalChunks)
        return;
...
// Transfer Summary
// ============================================================================
function showTransferSummary(direction, fileCount, totalSize, durationMs) {
