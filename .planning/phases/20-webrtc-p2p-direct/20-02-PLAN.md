...
must_haves:
  truths:
    ...
    - "When --no-p2p is set or proxy is active, no candidate gathering or hole punch attempt occurs"
  ...
  key_links:
    ...
    - from: "crates/tallow-net/src/transport/p2p.rs"
      ...
      via: "DirectListener/connect_direct for QUIC hole punch"
...
use crate::transport::direct::{DirectListener, DirectConnection, connect_direct};
...
///
/// This function:
/// 1. Detects NAT type (skip if symmetric)
/// 2. Gathers local candidates
/// 3. Exchanges candidates with peer via the relay channel
/// 4. Attempts QUIC hole punch to highest-priority remote candidate
/// 5. Returns DirectConnection on success or fallback reason on failure
///
...
async fn negotiate_inner(
    ...
    // Initiator (sender) = QUIC client: connect to remote candidates
    // Responder (receiver) = QUIC server: accept on the listener
    let result = if is_initiator {
        attempt_as_client(&valid_candidates).await
    } else {
...
async fn attempt_as_client(
    ...
    for candidate in candidates {
        ...
        match connect_direct(candidate.addr, HOLE_PUNCH_ATTEMPT_TIMEOUT).await {
...
**7. If `DirectListener::bind()` does not return an endpoint that also functions as a client:**

The existing `DirectListener` only creates a server endpoint. For hole punching, we might need the listener's endpoint to also have a client config for the initiator path. Check if `connect_direct()` creates its own endpoint (it does -- line 271 of direct.rs). This means the initiator path uses a separate endpoint, which is fine for Phase 20. The server path (responder) uses the listener. No changes to `direct.rs` needed unless the QUIC endpoint needs both client and server config on the same socket for proper NAT pinhole creation.

**Important:** If the initiator's `connect_direct()` binds to `0.0.0.0:0` (a random port), the NAT pinhole created by STUN on the listener's port won't help. To fix this: modify `connect_direct()` or add a `connect_direct_from_port(remote_addr, local_port, timeout)` variant that binds the client endpoint to the SAME port as the listener. This ensures the NAT mapping discovered by STUN matches the outbound connection.

Add to `direct.rs`:

```rust
/// Connect directly to a peer via QUIC, binding from a specific local port.
///
/// Used for hole punching: the local port must match the STUN-discovered
/// mapping so the NAT pinhole is reused.
#[cfg(feature = "quic")]
pub async fn connect_direct_from_port(
    ...
    let mut endpoint = quinn::Endpoint::client(bind_addr)
        .map_err(|e| NetworkError::ConnectionFailed(format!(
            "direct client bind to port {} failed: {}", local_port, e
        )))?;
...
Then update `attempt_as_client()` in p2p.rs to use `connect_direct_from_port()` when a local port is known, passing the listener's port.
...
Run: `powershell.exe -NoProfile -Command "& { $env:LIB = 'C:\Program Files (x86)\Windows Kits\10\Lib\10.0.26100.0\um\x64;C:\Program Files (x86)\Windows Kits\10\Lib\10.0.26100.0\ucrt\x64;C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Tools\MSVC\14.43.34808\lib\x64'; $env:INCLUDE = 'C:\Program Files (x86)\Windows Kits\10\Include\10.0.26100.0\um;C:\Program Files (x86)\Windows Kits\10\Include\10.0.26100.0\ucrt;C:\Program Files (x86)\Windows Kits\10\Include\10.0.26100.0\shared;C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Tools\MSVC\14.43.34808\include'; $env:PATH = 'C:\Program Files\Microsoft Visual Studio\2022\Community\VC\Tools\MSVC\14.43.34808\bin\Hostx64\x64;' + $env:PATH; cargo build --workspace 2>&1; cargo clippy --workspace -- -D warnings 2>&1 }"` -- builds clean, clippy clean.
  ...
  <done>
`p2p.rs` module provides complete `negotiate_p2p()` function. `connect_direct_from_port()` added to direct.rs. Transport mod.rs exports P2P types. Compiles and passes clippy.
  </done>
</task>
...
if !is_direct && proxy_config.is_none() && !args.no_p2p {
    ...
    match tallow_net::transport::negotiate_p2p(&mut channel, true).await {
...
Find the equivalent `// --- End handshake ---` or `"Secure session established"` message in receive.rs and insert the same block with ONE difference: the receiver is the responder (QUIC server role):

```rust
// Receiver = responder (QUIC server role)
match tallow_net::transport::negotiate_p2p(&mut channel, false).await {
```

Everything else is identical to send.rs.
...
**5. Verify the insertion point in receive.rs.** Look for the pattern after HandshakeComplete is sent (around lines 260-280 in receive.rs). The P2P upgrade block goes after the session key is derived but BEFORE `pipeline.set_session_key()` or the FileOffer receive loop.
...
4. `direct.rs` has `connect_direct_from_port()` for port-bound QUIC connections
...
- Sender uses client role (connect_direct), receiver uses server role (accept_peer)
