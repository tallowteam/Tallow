...
<task type="auto">
  ...
  **Step 3: Create web/app.ts — Main application logic**
  ...
  3. Open WebSocket: `new WebSocket('wss://relay.tallow.app/ws')` (or configurable relay URL)
  ...
  **Sliding window:** For v1, send all chunks sequentially (wait for each ACK is too slow, fire-and-forget is simplest). The CLI uses window=8 but the browser can start simpler. Send chunks in bursts of 8, wait for ACKs, then send next 8. This matches the CLI's sliding window behavior.
  ...
  <verify>
  The web files exist and are syntactically valid. `cargo check -p tallow-web` still passes with the new modules. Open index.html in a browser — the landing page renders with code phrase input and Send/Receive buttons. The drag-and-drop zone visually highlights on dragover. CSS renders correctly in both light and dark system themes.
  </verify>
  <done>
