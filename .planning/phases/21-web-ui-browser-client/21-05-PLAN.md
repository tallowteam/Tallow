...
must_haves:
  truths:
    ...
    - "WASM crypto produces identical output to native for KEM, AES-GCM, and BLAKE3 — verified by cargo test on native target (same Rust code compiles to both native and WASM)"
    - "WebSocket relay length-prefix bridging is correct — unit tests verify add/strip of 4-byte prefix (full bidirectional WS-QUIC forwarding requires integration test with live relay, not covered here)"
    - "Build instructions document end-to-end WASM compilation pipeline (cargo build + wasm-bindgen + optional wasm-opt)"
  artifacts:
