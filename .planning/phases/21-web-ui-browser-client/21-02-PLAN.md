...
must_haves:
  truths:
    ...
    - "WSS (TLS) is supported either natively (axum-server tls-rustls with cert/key config) or via Caddyfile reverse proxy — browsers on HTTPS can connect without mixed content errors"
  artifacts:
    ...
    - path: "crates/tallow-relay/src/config.rs"
      provides: "ws_port, ws_enabled, ws_tls_cert, ws_tls_key configuration fields"
    - path: "deploy/Caddyfile.web"
      provides: "Caddy reverse proxy config for WSS termination"
  key_links:
...
<task type="auto">
  ...
  **Step 4: WSS (TLS) configuration for production deployment**

  Browsers served over HTTPS cannot connect to `ws://` (mixed content blocked). The WebSocket endpoint MUST support WSS in production. Two approaches — implement BOTH for flexibility:

  **Approach A — Reverse proxy (recommended for deployment):**
  Create `deploy/Caddyfile.web` with Caddy reverse proxy configuration:
  ```
  web.tallow.app {
      reverse_proxy /ws localhost:4434
  }
  ```
  This gives WSS termination at the edge with automatic Let's Encrypt certificates. Document in `deploy/WEB-DEPLOY.md`.

  **Approach B — Native TLS on axum (for standalone relay):**
  Add optional `ws_tls_cert` and `ws_tls_key` fields to `RelayConfig`. When provided, use `axum_server::tls_rustls::RustlsConfig` (or `rcgen` for self-signed dev certs) to serve WSS directly:
  ```rust
  if let (Some(cert), Some(key)) = (&config.ws_tls_cert, &config.ws_tls_key) {
      let tls = RustlsConfig::from_pem_file(cert, key).await?;
      axum_server::bind_rustls(ws_addr, tls).serve(router.into_make_service()).await?;
  } else {
      axum::serve(tcp_listener, router).await?;
  }
  ```
  Add `axum-server = { version = "0.7", features = ["tls-rustls"] }` to tallow-relay dependencies.
  Config defaults: `ws_tls_cert: None`, `ws_tls_key: None`. When both are None, plain WS is used (suitable behind a reverse proxy).

  **Step 5: Update web frontend relay URL default**

  The frontend's default relay URL in app.ts MUST use `wss://` (not `ws://`) for production:
  ```typescript
  const DEFAULT_RELAY = 'wss://129.146.114.5:4434/ws'; // or wss://relay.tallow.app/ws
  ```
  Allow override via URL parameter `?relay=wss://custom-relay/ws` or `?relay=ws://localhost:4434/ws` for local dev.
...
- WSS is configurable via ws_tls_cert/ws_tls_key or deployable behind Caddy reverse proxy
</success_criteria>
