...
<task type="auto">
  ...
  **Step 1: Create clipboard.rs — Clipboard content handling in WASM**
  ...
  2. **Clipboard message encoding:**
     - `pub fn encode_clipboard_message(session_key: &[u8], transfer_id: &[u8], text: &[u8]) -> Result<Vec<u8>, JsValue>` — Encrypts the clipboard text with AES-256-GCM (nonce counter = 0, AAD = "clipboard" domain separator + transfer_id), encodes as the appropriate Message variant (check which Message variant the CLI's `tallow clip` uses — likely Message::Chunk with a special manifest indicating clipboard content, or a dedicated variant if one exists).

     IMPORTANT: Look at how the CLI's `tallow clip` command sends clipboard content over the wire. It likely uses the standard FileOffer + Chunk pipeline with a TransferType indicator. The browser must use the identical wire format. If the CLI sends clipboard as a virtual file (which is the likely pattern from Phase 16), then encode_clipboard_message should produce a FileOffer with the appropriate manifest + a single Chunk containing the encrypted content.
  ...
  **Step 2: Create chat.rs — Chat message handling in WASM**
  ...
  1. **Chat message encryption:**
     ...
     Nonce construction: Use counter-based nonce with chat domain separation. The nonce must be 12 bytes (96 bits) for AES-256-GCM. Build as: `sequence` as 8-byte LE in the last 8 bytes of the 12-byte nonce, with the first 4 bytes as a domain separator (e.g., 0x00000001 for chat).

     CRITICAL: Check how the CLI's chat module builds nonces in `tallow-protocol/src/chat/`. The browser MUST use the identical nonce construction. If the CLI stores the nonce in `ChatText.nonce`, then the browser must produce the same nonce bytes.
